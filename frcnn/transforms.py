...
class FixedSizeCrop(nn.Module):
    ...
    def forward(self, img, target=None):
        ...
        if new_height != height or new_width != width:
            ...
            r0 = torch.rand(1)
            r1 = torch.rand(1)
            top = int(offset_height * r0)
            left = int(offset_width * r1)
...
def _copy_paste(
    ...
    if size1 != size2:
        paste_image = F.resize(paste_image, size1)
        paste_masks = F.resize(paste_masks, size1)
        # resize bboxes:
        ratios = torch.tensor(
...
class SimpleCopyPaste(torch.nn.Module):
    def __init__(self, blending=True):
        super().__init__()
    ...
    def __repr__(self) -> str:
        s = f"{self.__class__.__name__}(blending={self.blending})"
        return s
